...
model_name = 'tf.keras-8574BB6B-931E-452D-BA8A-739C6C730AFF-epoch-00003-val_accuracy-0.9815312623977661'
...
app = Flask(__name__)
api = Api(app, version='1.0', title='REST API', doc='/__swagger__', description='A sample API for demonstration')
app.config.SWAGGER_UI_DOC_EXPANSION = 'list'
ns_conf = api.namespace('Fraud-Detection-deploy', description='AI Model Deploy Example')
...
class ConferenceList(Resource):
    ...
    def post():
        ...
        json_file = open(model_name+'.json', 'r', opener=opener)
        ...
        loaded_model.load_weights('runs/best-model/'+ model_name +'.h5')
        if upload_parser.parse_args():
            ...
            test_y = pd.read_csv(label)
            test_y = test_y.iloc[0,0]

            test_pred  = loaded_model.predict_classes(test_data)

            result = 'target: {},  predicted value : {}'.format( test_y , test_pred[0,0])
            return result
        else:
...
def custom_alarm():
    """
        ...
        required return value:
            ...
            notifiers: list of notifier (dictionary format). web push is default notifier
                provided notifier = slack
                required input value to use notifier
                    slack: hook_url
                    sample
                        notifiers['slack'] = {'hook_url': '{hook_url}'}
    """
###################################################################################################
    # Do not modify this code block #
...
###################################################################################################
    ...
    # Set notifiers
    # notifiers['slack'] = {'hook_url': ''}
###################################################################################################
    # Do not modify this code block #
    # Return message and notifiers
    return message, notifiers
###################################################################################################
# Do not modify this code block #
